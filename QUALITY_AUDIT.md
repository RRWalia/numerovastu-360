...
Last reviewed: 2026-09-19 — Release 2.14.1 Print-dossier collision polish on the 2.14.0 Dual-name intake (badge boxes that grow with wrapped labels instead of overprinting the line below, defined 1.35 leading for the Section 6 and Section 14 tables, block-stacked cell titles, complete Sunday dressing guidance)
...
| Dasha coverage to 75/80 | Added (2.11.0) | Wealth & business event windows now scan **21–75** (was 21–70): career to 70, property to 75, abroad to 60, marriage to 45. Every Dasha horizon assumes an **80-year** average lifespan; late significator windows are scanned till age 80 or 15 years ahead, whichever reaches further. Each event prints five detailed upcoming windows (was three), three late windows once a band closes (was two), plus a **Lifetime coverage** line naming total windows in band and the furthest one, so a 75-year band can no longer read as “covered only till 56”. Pack v2.9.0 (data + thresholds), app 2.11.0. Smoke pins the 21–75 scan and the coverage line. |
| Partner Chandra-bala Rashi axis | Added (2.12.0) #46 | Partner intake now captures optional **partnerBirthTime** and **partnerBirthPlace**, wired to the same `searchPlaces()` atlas autocomplete as the primary birthplace so vocabularies cannot drift. Both values round-trip through snapshot and form. Engine `chandraBala()` computes the partner's sidereal Moon via the on-device Meeus engine and reports the mutual Rashi axis: **1/1 Ekarashi, 2/12 Dwidwadasha, 3/11, 4/10, 5/9 Navapanchama, 6/8 Shadashtaka, 7/7 Samasaptaka**. Where both Moons share a rashi lord — classical cancellation of Shadashtaka/Dwidwadasha — that is reported as a fact, not applied as a score, because schools differ. Two states only: Tier 1 (date only) shows Driver/Conductor comparison and a banner naming which side is missing; Tier 2 (time+place both sides) shows the computed verdict. No noon default, no stand-in city, no approximate Moon — returns `{ tier: 1, chandraBalaComputed: false, missing, message }`. Deliberately **not** a 36-point Ashtakoota score (Gana/Nadi/Yoni/Graha Maitri not computed/implied) and prescribes nothing. `data-authority=\"chandra-bala\"`, EN/HI/GU. Smoke pins degradation contract, all 12 relative Moon positions vs 7 axes, shared-lord staying score-free, and mutation: default noon/Delhi turns suite red. |
...
| Micro-Routine cell formatting | Fixed (2.13.1) polish #1 | The 7-Day Micro-Routine action cell used to concatenate its three sources into one sentence ("Wear white. Wear clean, fragrant clothes Donate: Donate white sweets…"). `microActionCell()` now renders labelled bullets — **Wear:** / **Action:** / **Donate:** — as a `ul.micro-action-list[data-micro-action="steps"]` with one `li[data-action-kind]` per slot, the `Donate` prefix is stripped before the label so it can never double, and a lifestyle clause that only repeats the colour slot ("Wear green on Wednesdays" beside `Wear: Green`) is dropped. Rows stay keyed to `dayOf(n)`, the Sunrise row still matches the Tier-1 practice target, and EN/HI/GU labels all localise. Pinned on the Friday and Wednesday rows. |
| Solar-mute clarity | Fixed (2.13.1) polish #2 | "Muted — run gently" beside a Sun row that still read "Offer water to the rising Sun daily" left the client unable to tell whether their daily offering was now banned. The muted Sun row now prints the permitted practice explicitly (**"Brief sunrise Arghya only (30 sec); avoid midday sun and skip Surya Bhedana"**) and drops the long daily-offering clause; `renderDoAvoidCards()` adds a DO item ("Keep the sun practice minimal: a brief 30-second sunrise Arghya only, then cool down with Chandra Bhedana or evening grounding") and narrows the DO-NOT item to the extended forms. The row keeps `data-conflict-muted="solar-overload"` and the muted note for the automated audit, but carries the new `solar-guardrail-row` tint instead of the blanket strikethrough — striking through a permitted practice read as a prohibition. |
| Client-text hygiene / dangling hyphen | Fixed (2.13.1) polish #3 | The reported `clutterfree-;` typo does not exist in any source string (the canonical zone remedy is `"Keep the North-West clutter-free; add brass decor and check stored water."`), so it is a print/extraction artefact of a wrapped hyphenated compound. `tidyText()` now normalises every data-driven cell before it reaches markup: known compounds are repaired, a hyphen touching punctuation is dropped, spaced or doubled punctuation and repeated spaces collapse. The 90-day Pratyantar Vastu micro-action additionally prints clause-per-line (`ul.micro-forecast-list`, one complete sentence per bullet) so no wrapped line can strand a hyphen, and `.micro-routine-table td`, `.micro-forecast-table td` and `.spelling-table td` set `hyphens: none`. A smoke assertion scans the whole rendered client report for `clutterfree` and for any `letter-;` / `letter-,` / `letter-.` artefact. |
| Tiered name menu with Strategy column | Fixed (2.13.1) polish #4 | The 2.13.0 pass replaced double-consonant options with two middle initials, which silently removed a strategy the client might prefer. Section 6 now prints BOTH in one table with a **Strategy** column: `initial` → **Middle Initial** (legal/banking clean), `double`/`swap`/`insert` → **Spelling Alteration** (social, digital and card use). The rating is tiered by strategy instead of blanket-penalising doubles — middle initial 5 Excellent, low-contrast double (`Amar Sambhhvani`) 4 Good, ending-shifting double (`Amar Sambhvanni`) 3 Moderate, same-sound swap 4, mid-name vowel insert 3, ending vowel insert (Suniel pattern) 4. `buildOptionalSpellings()` uses `tieredSpellingMenu()` (initials first, alterations underneath, per-tier caps) and the correction path uses `pairedSpellingPick()` (one legal-safe + one digital-safe per target). `nameStrategyOf()` labels/hints localise EN/HI/GU and print once per strategy group; `data-strategy`, `data-spelling-strategy`, `data-change` and `data-new-number` stay on the markup for audits. The reference chart renders exactly **Amar E Sambhvani · Amar H Sambhvani · Amar Sambhhvani · Amar Sambhvanni**. |
| Badge/cell print collision (Section 6) | Fixed (2.14.1) polish #1 | The reported `Window 1 - Everyday / Buyhliess cards` / `Formal / Passpment` overlap on dossier pages 20–21 was never bad text: `.badge` carried a FIXED `height: 22px` as an inline-flex box, so a wrapped window/strategy label overflowed its own box and the second line printed on top of the description beneath it. Badges now grow (`min-height: 22px`), `.spelling-table td` runs on defined `line-height: 1.35; vertical-align: top`, and each Strategy / Applies-to cell renders its badge title inside a dedicated `div.spelling-cell-title` block, title and description as two boxes. Pinned by smoke (markup structure + CSS contract). |
| Micro-forecast math-line collision (Section 14) | Fixed (2.14.1) polish #2 | Page 41's `jointbone /strain` and the arrow bleeding into `MD 5 · AD 9` were the same collision in the 90-day Pratyantar table (the `Friction with AD` / `New AD n · Planet` badges shared the sub-lord line box). The data string was verified already canonical (`data.js` dasha 8 caution: “Delays, joint/bone strain and pessimism; …”); the fix is layout: the shared badge growth applies, `.micro-forecast-table td` gets the same `1.35 / top` leading, and the `MD n · AD n` math prints as its own block under the sub-lord `<strong>`. Pinned by smoke (source string + block structure). |
| Sunday dressing guidance (Layer 1) | Fixed (2.14.1) polish #3 | The Sunday row read `Wear: Gold` while Friday carried full guidance, because the Wear slot took only the first token of the colour list. `numbers[1]` (base DB + all three i18n packs + re-derived 2.9.0 pack) now ships a dedicated `wearGuidance` line — EN “Gold, saffron, or warm yellow” / HI “सुनहरा, केसरिया या हल्का पीला” / GU “સોનેરી, કેસરીયું કે હળવું પીળું” — preferred by the Micro-Routine Wear slot for any number that defines it; all other weekdays keep the previous first-colour extraction byte-for-byte. Pinned by smoke. |
...
**Pillar 5 — 2.13.1 formatting & client-agency polish (post-review).**
- `microActionCell()` labelled bullets; the Donate prefix never doubles and a
  wear clause that repeats the colour slot is dropped.
- Solar guardrail symmetry: the muted Sun row states the permitted 30-second
  sunrise arghya, the DO card carries it, the DO-NOT card bans only the
  extended forms, and the row is tinted rather than struck through.
- `tidyText()` client-text hygiene plus clause-per-line Pratyantar micro-actions
  and `hyphens: none` on the three affected table classes.
- `tieredSpellingMenu()` / `pairedSpellingPick()` restore the spelling-alteration
  strategy beneath the middle initials, with a Strategy column and a rating
  tiered by real-world use (5 / 4 / 3) instead of a blanket double penalty.
