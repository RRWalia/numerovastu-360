...
1. **Commit reviewed pixel baselines.** The three pixel tests now exist and
   pass; they need baselines generated on the CI image (Playwright's Chromium on
   `ubuntu-latest`), not on a developer machine — cross-renderer and cross-font
   baselines would poison the gate. A dedicated
   `.github/workflows/visual-baselines.yml` (`workflow_dispatch`) generates and
   commits them; until it has run, the visual job warns instead of comparing.
